...
layout.show(nf)
par(mar=c(0.0,0.1,0.1,0.1),oma=c(2.5,3,0,0.5),mai=c(0.0,0.05,0.05,0) )
...
axissize <- 1.2
x.label = 'Year'
y.label = expression(paste('Growth rate (K ;',year^-1,')'))
...
nf <- layout(matrix(c(1:1),nrow=1,ncol=1,byrow=TRUE),  TRUE) 
layout.show(nf)
par(mar=c(0.0,0.1,0.1,0.1),oma=c(3.0,3.0,0.4,0.5),mai=c(0.0,0.05,0.0,0) )
...
legend(1977, 0.72, c("Lake 1" ,
                     "Lake 2" ,
                     "Lake 3" ,
                     "Lake 4" ,
                     "Lake 5" ),
       lty=c(1,1,1,1,1,1 ), col=c(gcols[1],gcols[5],gcols[3],gcols[4],gcols[7],gcols[8]),
       lwd=c(lwd1,lwd1,lwd1,lwd1, lwd1), cex=0.9)

mtext(y.label, line = 1.0, side = 2, cex = size.text,outer=T)
mtext(x.label, line = 1.5, side = 1, cex = size.text, outer=T)
